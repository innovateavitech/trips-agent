...
| 8 | **The invitation-accept route hashed a password before it looked the token up** (issue 173, item 2). Argon2id is expensive on purpose, so an anonymous caller could spend that CPU by posting any string as a token — and the route named no rate-limit policy, so it fell back to the default per-address one. | Low | The token is looked up first, and only a link that turns out to be real is paid for. The route has a policy of its own, `InvitationAccept`: ten in fifteen minutes per address. `RateLimitingTests`, `SubAgentNetworkEndToEndTests` |
