...
| `feat/M3-subagents` | 3 | F10 whole (#63): invitations, scopes, permission overrides and margin visibility, race-free wallet allowances, freeze and revoke, consolidated network reporting, and four console screens. On top of the admin console branch, with PR 2's `main` merged in | pushed, done |
| — | 3, 4 | F11-F14 | not started |

**Next:** PR 2 is open from `feat/M2-commerce`. When it merges, re-check decision 14's storefront half (the admin console's `StorefrontAvailability` belongs in `PublicSiteResolver`), then assemble PR 3 from the admin console, billing and sub-agent branches, with analytics, payouts and the loyalty flag still to build. When billing lands, `ISubAgentEntitlement` needs an implementation that reads `max_sub_agents`; until then `UnlimitedSubAgentEntitlement` allows everything, and swapping it is one line in `DependencyInjection.AddApplication`. `feat/M2-commerce` already carries the storefront and CRM merges, so it is the branch to merge the others into.
