...
public sealed class PublicStorefrontEndpointTests : IClassFixture<RedisFixture>, IAsyncLifetime, IDisposable
{
    ...
    public async Task InitializeAsync()
    {
        ...
        await using (var setup = await _postgres.CreateEmptyDatabaseAsync(_database, tenancy.Tenant, tenancy.Scope))
        {
            ...
            // Suspending demands a reason since the admin console landed (#66): a status change
            // that nobody can explain is exactly what the audit trail exists to prevent.
            shut.Suspend("Suspended for this test", DateTimeOffset.UtcNow);
